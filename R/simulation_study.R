...
dat2 <- dat %>% mutate(
 ...
 timeplotExample <-  dat2 %>% 
    ...
    scale_y_continuous(name = "Value", breaks = seq(0,150,2))+
  ...
  sol <- solve.QP(Dmat = cov(dat2[,c("x","y")]), 
                  dvec = rep(0, 2),
                  Amat = Amat[,-2],
                  bvec = c(1,#mu[1],
                           rep(0,length(mu))),#rangemu[which.min(js)]), 
                  meq  = 1)
  sol.clim <- solve.QP(Dmat = matrix(
    ...
    bvec = c(1,#mu[1],
             rep(0,length(mu))),#rangemu[which.min(js)]), 
    meq  = 1)
  sol.noclim <- solve.QP(Dmat = cov(dat2[,c("x","y")])-matrix(
    c(r1^2, r1*r2*cos(phi1),
      r1*r2*cos(phi1),r2^2)/2,
    ncol=2, nrow=2), 
    dvec = rep(0, 2),
    ...
    bvec = c(1,#mu[1],
             rep(0,length(mu))),#rangemu[which.min(js)]), 
    meq  = 1)
  weights <- rbind("Empirical" = sol$solution,
  ...
  dat3 <- dat2 %>% 
    mutate("Empirical" = weights$X[1]*x + weights$Y[1]*y,
    ...
    mutate(source = factor(source, 
                           levels = c("Empirical", "Seasonal", "Seasonal-adjusted"))) 
...
(  timeplot <- dat3 %>% 
    ...
    ggplot(aes(x=date, y= power,group = source)) + geom_line(col = "blue")+
    ...
    geom_line(data= fitted %>% filter(year(date)%in%2013:2015),
              aes(x = date, y = .fitted), lty = 2, lwd = .9)+
